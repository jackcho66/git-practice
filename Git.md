...
以下資料皆整理自[程式與網頁開發者必備技能！Git 和 GitHub 零基礎快速上手，輕鬆掌握版本控制的要訣！](https://www.youtube.com/watch?v=FKXRiAiQFiY&t=85s "游標顯示")

- [介紹Git的操作](#介紹git的操作)
  - [1.安裝完Git後，可開啟vscode檢查是否正確安裝](#1安裝完git後可開啟vscode檢查是否正確安裝)
  - [2.使用Git前必須先設定姓名和電子郵件，因為Git在記錄版本變更時也會記錄作者資訊](#2使用git前必須先設定姓名和電子郵件因為git在記錄版本變更時也會記錄作者資訊)
  - [3.把目前資料夾轉成具有版本功能的儲存庫](#3把目前資料夾轉成具有版本功能的儲存庫)
  - [清空指令](#清空指令)
  - [檢查當前目錄中的檔名和狀態](#檢查當前目錄中的檔名和狀態)
  - [將未追蹤的檔案放到放到暫存區](#將未追蹤的檔案放到放到暫存區)
  - [紀錄版本變更](#紀錄版本變更)
  - [檢視歷史紀錄](#檢視歷史紀錄)
  - [比較新舊版本內容差異與還原](#比較新舊版本內容差異與還原)
  - [目錄新增和刪除文件](#目錄新增和刪除文件)
  - [忽略檔案清單](#忽略檔案清單)
  - [將專案上傳至Github](#將專案上傳至github)
  - [將本地內容更新上傳Github](#將本地內容更新上傳github)
  - [邀請其他人參與專案](#邀請其他人參與專案)
  - [下載他人儲存庫到本地端電腦使用](#下載他人儲存庫到本地端電腦使用)
  - [建立分支](#建立分支)
  - [發起合併請求](#發起合併請求)
  - [同意合併請求](#同意合併請求)
  - [學習心得分享](#學習心得分享)
...
此時回到Github頁面時會發現branch分支頁面變成2個
...
分支使用者可以發起合併請求。
點擊進入`Branches頁面`->按下`...`->`New pull request`。
...
點擊主頁左上角的`Pull requests`，即可查看合併申請。

![alt text](images/image-39.png)

審核過程中發現需要修改的地方可以在評論區留言。

若沒有問題就點擊`Merge pull request`->`confirm merge`。
同意完後分支就完成他的任務，可以安心刪掉了。

至於本地端的檔案要執行`git pull`使本地與遠端的儲存庫維持同步。
